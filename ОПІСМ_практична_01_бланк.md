...
| - | --------------------------------- | -------------------------------- | ------------- |
| 1 | Результат | `<!DOCTYPE html>`<br>`</html>`<br>HTML-код отриманої сторінки | Готовий код і текст вебсторінки, який у результаті отримує та бачить користувач. |
| 2 | HTTP/HTTPS (Запит і відповідь) | `> GET / HTTP/1.1`<br>`> Host: opensuse.org`<br>`> User-Agent: curl/8.13.0`<br>`> Accept: */*`<br>`* Request completely sent off`<br>`< HTTP/1.1 301 Moved Permanently`<br>`< HTTP/1.1 200 OK`<br>`< Location: https://www.google.com/`<br>`< Content-Type: text/html; charset=UTF-8`<br>Аналогічні рядки для `google.com` та `neverssl.com` | Обмін командами та даними для отримання вебсторінки. Тут показано HTTP-запит клієнта та відповідь сервера. |
| 3 | TLS/SSL (Перевірка сертифікатів та шифрування) | `* schannel: disabled automatic use of client certificate`<br>`* ALPN: curl offers http/1.1`<br>`* ALPN: server accepted http/1.1`<br>`* using HTTP/1.x`<br>`* schannel: remote party requests renegotiation`<br>`* schannel: renegotiating SSL/TLS connection`<br>`* schannel: SSL/TLS connection renegotiated`<br>`SEC_E_CERT_EXPIRED`<br>`The received certificate has expired.`<br>`SEC_E_WRONG_PRINCIPAL`<br>`The target principal name is incorrect.`<br>`SEC_E_UNTRUSTED_ROOT`<br>`The certificate chain was issued by an authority that is not trusted.` | Перевірка сертифіката сайту, узгодження параметрів TLS та захист даних під час з'єднання. |
| 4 | DNS (Перетворення доменного імені в IP-адресу) | `* Host opensuse.org:443 was resolved.`<br>`* IPv6: (none)`<br>`* IPv4: 195.135.223.50`<br>`opensuse.org AAAA ... 2a07:de40:b27e:1204::10`<br>`opensuse.org A ... 195.135.223.50`<br>Аналогічні рядки для `google.com`, `neverssl.com` та `badssl.com` | Перетворює зрозуміле користувачу доменне ім'я на числову IP-адресу, за якою можна встановити з'єднання із сервером. |
| 5 | Порти (Підключення до сервера) | `* Trying 195.135.223.50:443...`<br>`* Trying 34.223.124.45:80...`<br>`* Connected to opensuse.org ...`<br>`* Connected to neverssl.com ...`<br>`* Connected to google.com ...` | Встановлення зв'язку з потрібним сервером через мережевий порт. Ця група є найближчою до апаратного забезпечення серед наведених у виводі `curl`. |
